...
class ViewController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        self.currencyIndex = row
        let selectedCurrency: String = currencyArray[self.currencyIndex]
...
//    
    func updateBitcoinPrice(json: JSON) {
        if let tempResult = json["last"].double {
            bitcoinPriceLabel.text = String(format: "\(currencySymbolArray[self.currencyIndex]) %.2f", tempResult)
        } else {
